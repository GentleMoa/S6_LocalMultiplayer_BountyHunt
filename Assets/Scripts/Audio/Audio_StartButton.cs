...
public class Audio_StartButton : MonoBehaviour
{
    ...
    //Private Variables
    private AudioClip _audio_startButton_sound;
    ...
    void Start()
    {
        ...
        AudioManager.Instance.AE_StartButton_Sound += Play_Audio_StartButton_Sound;

        //Get the AudioClip from the Settings
        _audio_startButton_sound = Settings.Instance.audio_startButton_sound;
    }

    public void AudioCondition()
    {
        //Audio Condition (if the enemy gets enabled) --> Tell the AudioManager to shoot the Audio Event
        AudioManager.Instance.ShootAudioEvent_StartButton_Sound();
    }

    private void Play_Audio_StartButton_Sound()
    {
        //Input AudioClip into AudioSource and play it
        _audioSource.clip = _audio_startButton_sound;
        _audioSource.Play();
    }
